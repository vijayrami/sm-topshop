...
class Sm_Imageslider_Helper_Data extends Mage_Core_Helper_Abstract
{
	const XML_ENABLED_IMAGESLIDER  = 'imageslider_cfg/general/active';
	const XML_INCLUDE_JQUERY  = 'imageslider_cfg/advanced/include_jquery';

	public function enabledImageslider($store = null)
	{
		return Mage::getStoreConfigFlag(self::XML_ENABLED_IMAGESLIDER, $store);
	}
	...
	{
		if (!(int)$this->enabledImageslider()) return;
		if (!defined('MAGENTECH_JQUERY') && (int)$this->getIncludeJquery()) {
			define('MAGENTECH_JQUERY', 1);
			$_jquery_libary = 'sm/imageslider/js/jquery-2.1.3.min.js';
	...
	public function getInlucdeNoconflict()
	{
		...
		if (!defined('MAGENTECH_JQUERY_NOCONFLICT') && (int)$this->getIncludeJquery()) {
			...
			$_jquery_noconflict = 'sm/imageslider/js/jquery-noconflict.js';
			return $_jquery_noconflict;
		}
	}

	public function addJsOwlCarousel()
	{
		if (!(int)$this->enabledImageslider()) return;
		$_css = 'sm/imageslider/js/owl.carousel.js';
		return $_css;
	}
	/**
