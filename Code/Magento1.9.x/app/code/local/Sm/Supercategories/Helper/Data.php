...
class Sm_Supercategories_Helper_Data extends Mage_Core_Helper_Abstract {

	const XML_ENABLED_SUPERCATEGORIES  = 'supercategories_cfg/general/active';
	const XML_INCLUDE_JQUERY  = 'supercategories_cfg/advanced/include_jquery';

	public function enabledSuperCategories($store = null)
	{
		return Mage::getStoreConfigFlag(self::XML_ENABLED_SUPERCATEGORIES, $store);
	}
	...
	{
		if (!(int)$this->enabledSuperCategories()) return;
		if (!defined('MAGENTECH_JQUERY') && (int)$this->getIncludeJquery()) {
			define('MAGENTECH_JQUERY', 1);
			$_jquery_libary = 'sm/supercategories/js/jquery-2.1.3.min.js';
	...
	public function getInlucdeNoconflict()
	{
		...
		if (!defined('MAGENTECH_JQUERY_NOCONFLICT') && (int)$this->getIncludeJquery()) {
			...
			$_jquery_noconflict = 'sm/supercategories/js/jquery-noconflict.js';
			return $_jquery_noconflict;
		}
	}

	public function getInlucdeSlider()
	{
		if (!(int)$this->enabledSuperCategories()) return;
		if (!defined('MAGENTECH_JQUERY_SLIDER') && (int)$this->getIncludeJquery()) {
			define('MAGENTECH_JQUERY_SLIDER', 1);
			$_jquery_slider = 'sm/supercategories/js/slider.js';
			return $_jquery_slider;
		}
	}
	...
	public function _resizeImage($image, $config = array(), $type="product", $folder='resized'){
		...
		$_media_dir = Mage::getBaseDir('media') . DS . 'catalog' . DS .$type. DS;
