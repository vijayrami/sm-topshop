...
class Sm_Listingtabs_Helper_Data extends Mage_Core_Helper_Abstract
{

    public function getInlucdeJQquery()
    {
        if (!(int)Mage::getStoreConfig('listingtabs_cfg/general/active')) return;
        if (!defined('MAGENTECH_JQUERY') && (int)Mage::getStoreConfig('listingtabs_cfg/advanced/include_jquery')) {
            define('MAGENTECH_JQUERY', 1);
            $_jquery_libary = 'sm/listingtabs/js/jquery_1.11.3.js';
    ...
    {
        if (!(int)Mage::getStoreConfig('listingtabs_cfg/general/active')) return;
        if (!defined('MAGENTECH_JQUERY_NOCONFLICT') && (int)Mage::getStoreConfig('listingtabs_cfg/advanced/include_jquery')) {
            define('MAGENTECH_JQUERY_NOCONFLICT', 1);
            $_jquery_noconflict = 'sm/listingtabs/js/jquery-noconflict.js';
        ...
        foreach ($searchTags as $tag => $regex) {
            ...
            if (is_array($m) && isset($m[0]) && count($m[0])) {
                foreach ($m[0] as $htmltag) {
                    if(strlen(strstr($htmltag,'media')) > 0){
    ...
    private function getProductImages($product, $_config, $prefix = 'imgcfg')
    {
        ...
        if (count($mark) > 0) {
            ...
            foreach ($mark as $type => $true) {
                switch ($type) {
                    case 'product_image':
                        $image = ($product->getImage() != null) ? $product->getImage() : ($product->thumbnail != null ? $product->small_image : '');
                    ...
                    case 'category_description':
                        ...
                        if (!empty($inline_images)) {
                            for ($i = 0; $i < count($inline_images); $i++) {
                                ...
                                if(strstr($inline_images[$i], 'media ')){
                                    $array1 = explode('url="',$inline_images[$i]);
                                    $array2 = explode('"}',$array1[1]);
                                    $url_media = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$array2[0];
                                    array_push($images_path, $url_media);
                                }else{
                                    if (file_exists($inline_images[$i]['src']) || @getimagesize($inline_images[$i]['src']) !== false) {
                                        array_push($images_path, $inline_images[$i]['src']);
                                    }
