...
class Sm_BasicProducts_Block_List extends Mage_Catalog_Block_Product_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		if (!class_exists('Cache_Lite'))	
			require_once($this->getBaseDir() .  'lib' . DS .  'Sm' .DS . 'BasicProducts' .DS . 'Cache_Lite' . DS . 'Lite.php');
		$options = array(
		...
		if ($use_cache){
			...
			if (!empty($this->hash) && $data = $Cache_Lite->get($this->hash)) {
				...
				$template_file = (!empty($template_file)) ? $template_file : "sm/basicproducts/default.phtml";
