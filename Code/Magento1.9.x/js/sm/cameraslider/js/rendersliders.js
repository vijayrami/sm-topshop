...
var slideHideMasrginLeftRight = function slideHideMarginLeftRight() {
	var mg_l = $('cameraslider_position_margin_left');
	var mg_r = $('cameraslider_position_margin_right');
	var a = $('cameraslider_position_position_alignment').value;
	switch (a) {
		case 'topLeft':
		case 'topCenter':
		case 'topRight':
...
CameraSlide.prototype = {
			...
			if (b) {
				...
				if ("TABLE" === b.tagName) b.select("a").each(function(a) {
					...
					if (this.selectedLayer) {
						...
						this.selectedLayer.style.textTransform = this.selectedLayer.params.textTransform;
	...
	renderLayerHtml: function(a) {
		...
		switch (a.type) {
			...
			default:
				...
				b.style.textTransform = a.textTransform;
	...
	addLayer: function(a) {
		...
		if (void 0 == a.textTransform) a.textTransform = $("layer_textTransform").value;
	...
	searchVideo: function() {
		...
		if (c) {
			...
			switch (a) {
				...
				case "vimeo":
					jQuery.ajax({
						url: "http://vimeo.com/api/v2/video/" + c + ".json?callback=CmrSl.onVimeoCallback",
						...
						error: function(data){
							...
							b.insert('<p>No search video or video ID or URL you does not exist</p>');
							this.videoData = null;
						}
					});
			}
